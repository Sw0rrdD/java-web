...
public class TurnmoneyServlet  extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) {
		...
		int ToCardnumber = Integer.parseInt(tocardnumber);
		...
		System.out.println("Servlet: " + cardnumber + " " + tocardnumber + " " + superpassword + " " + getmoney);
		...
		try {
			if(!verificationcode.equalsIgnoreCase(localver)) {
				...
				messagesaction.AddTurnMessage(Cardnumber, ToCardnumber, Getmoney, 0);
				out = res.getWriter();
				...
				if(cardaction.TurnMoney(Cardnumber, ToCardnumber, Getmoney) == 1 ){
					messagesaction.AddTurnMessage(Cardnumber, ToCardnumber, Getmoney, 1);
					out = res.getWriter();
					out.print("<script>" + "alert('转账成功！');"
							+ "document.location.href='messages.jsp';"
							+ "</script>");
					out.close();
				}else{
					cardaction.InMoney(Cardnumber, Getmoney);
					messagesaction.AddTurnMessage(Cardnumber, ToCardnumber, Getmoney, 0);
					out = res.getWriter();
					out.print("<script>" + "alert('系统错误，转账失败！');"
							+ "document.location.href='messages.jsp';"
							+ "</script>");
					out.close();
				}
