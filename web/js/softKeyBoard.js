...
var softKeyBoardFn=function(){
	...
	this.pwInputChange=function(obj){
		Jid('passWord').value=obj.value;
	};
	
	this.addValue=function (newValue) {
		if (capsLockValue == 0) {
		   var str = Jid("passWord").value;
		   if (str.length < curPassWordInput.maxLength) {
			   Jid("passWord").value += newValue;
		   }
		   if (str.length <= curPassWordInput.maxLength) {
			   curPassWordInput.value = Jid("passWord").value;
		   }
		} 
		else {
		   var str = Jid("passWord").value;
	...
	this.setPassValue=function(){
		var longnum = Jid("passWord").value.length;
		var num;
		num = Jid("passWord").value.substr(0, longnum - 1);
		Jid("passWord").value = num;
		var str = Jid("passWord").value;
		curPassWordInput.value = Jid("passWord").value;
	};
		...
		for ( var i = 0; i < Jid("calc").elements.length; i++) {
		   if (Jid("calc").elements[i].type == "button" && Jid("calc").elements[i].getAttribute("bgtype") != "1") {
			   if (Jid("calc").elements[i].getAttribute("bgtype") == "2") {
				  Jid("calc").elements[i].className = "btn_num";
			   }
			   else {
				  Jid("calc").elements[i].className = "btn_letter";
			   }
...
function initSoftKeyBoard(id){
	Jid(id).onclick = function(){
			curPassWordInput=this;
			softFn.showKeyBoard();
			this.readOnly=1;
			Jid('passWord').value='';
	};	
	Jid(id).onKeyDown = function(){
		Jid('passWord').value=this.value;
	};
	document.getElementById(id).onfocus=function(){
					softFn.pwInputFocus(document.getElementById(id));
	};
	document.getElementById(id).onchange=function(){
					softFn.pwInputChange(document.getElementById(id));
	};
